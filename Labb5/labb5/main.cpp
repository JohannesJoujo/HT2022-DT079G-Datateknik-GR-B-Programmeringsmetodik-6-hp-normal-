...
*/
    ...
    if (it != bollvec.end()) {
        ...
        std::cout<<"There are no similar ball names and speed successively sequenced\n";
    }
    ...
    //std::vector<Ball>::iterator it;
    Ball needle[] = {{"Basketboll",   50.6},
                      {"Tennisboll",   110.8},
                      {"Fotboll",      100.2}};
    
    unsigned dataArraysize = sizeof(needle) / sizeof(Ball);
    it = std::search(bollvec.begin(), bollvec.end(), needle, needle + dataArraysize, searchseq());

    if (it != bollvec.end()) {
        std::cout << "needle2 subsequence found at position " << (it - bollvec.begin()) << '\n';
        MyPrint directutskrivning;
        //unsigned dataArraySize = sizeof(needle) / sizeof(Ball);
        directutskrivning(needle,dataArraysize);
        std::cout<<"\n";
    }
    else {
    ...
    //std::vector<Ball>::iterator it;
    double avg;
    double sum = std::accumulate(bollvec.begin(), bollvec.end(),double(),MyBinOp());
